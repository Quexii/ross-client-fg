...
public class HUDEditor extends RossScreen {
    ...
    protected void render(float mouseX, float mouseY, float partialTicks) {
        ...
        for (HUDModule module : modules) {
            module.dummy(new Hud.Layer(Hud.Layer.NAME_SKIA_BOTTOM));
        }
        skia.endFrame();

        for (HUDModule module : modules) {
            module.dummy(new Hud.Layer(Hud.Layer.NAME_VANILLA_BOTTOM));
        }

        skia.beginFrame();
        UI.use(skia);
        for (HUDModule module : modules) {
            module.dummy(new Hud.Layer(Hud.Layer.NAME_SKIA_TOP));
        }
        renderOverlay(modules, mouseX, mouseY);
        skia.endFrame();

        for (HUDModule module : modules) {
            module.dummy(new Hud.Layer(Hud.Layer.NAME_VANILLA_TOP));
        }
    }
    ...
    private void renderOverlay(List<HUDModule> modules, float mouseX, float mouseY) {
        ...
        try (Paint p = new Paint()) {
            ...
            if (dragging) {
                ...
                for (float x : snapLinesX) UI.drawLine(x, 0f, x, dh, p);
                for (float y : snapLinesY) UI.drawLine(0f, y, dw, y, p);
            }

            HUDModule hovered = moduleAt(modules, mouseX, mouseY);
            ...
            if (selected != null) {
                ...
                UI.drawCircle(Mth.clamp(ax, 5f, dw - 5f), Mth.clamp(ay, 5f, dh - 5f), 4f, p);
                p.setColor(theme.onAccent);
                UI.drawCircle(Mth.clamp(ax, 5f, dw - 5f), Mth.clamp(ay, 5f, dh - 5f), 1.5f, p);
            }
        }
    }
    ...
    private void drag(float mouseX, float mouseY) {
        ...
        if (!Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) && !Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
            ...
            candidatesY.add(dh);
            for (HUDModule other : ModuleManager.getEnabledHUDModules()) {
    ...
    protected void input(float mouseX, float mouseY, EventInput event) {
        if (event.type == EventInput.Type.MOUSE && event.value == 0) {
            if (event.action == EventInput.Action.PRESS) {
                HUDModule hit = moduleAt(ModuleManager.getEnabledHUDModules(), mouseX, mouseY);
                selected = hit;
                if (hit != null) {
                ...
                snapLinesX.clear();
                snapLinesY.clear();
                if (selected != null) selected.updateAnchor();
            }
        }
    ...
    private void nudge(float x, float y) {
        ...
        selected.setPosition(
                Mth.clamp(x, 0f, Display.getWidth() - bounds.width),
                Mth.clamp(y, 0f, Display.getHeight() - bounds.height));
        selected.updateAnchor();
    }

    private HUDModule moduleAt(List<HUDModule> modules, float x, float y) {
        for (int i = modules.size() - 1; i >= 0; i--) {
            Rect b = modules.get(i).getBounds();
